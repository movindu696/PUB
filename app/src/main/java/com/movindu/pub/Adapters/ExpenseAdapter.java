...
public class ExpenseAdapter extends ArrayAdapter<Expense> {
    ...
    public ExpenseAdapter(@NonNull Context context, @NonNull List<Expense> expenses) {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        if (convertView == null) {
            convertView = LayoutInflater.from(context).inflate(R.layout.report_item_expenses, parent, false);
        }

        Expense user_expenses = expenses.get(position);
        total = total + user_expenses.getAmount();

        TextView amount = convertView.findViewById(R.id.amountTextView);
        TextView cate = convertView.findViewById(R.id.cateTextView);
        ...
        cate.setText(user_expenses.getCategory().getName());
